...
async def run_pp(self, proxy):
    open("logs.txt", "a").write(f"\nV1 {proxy.current} [{time.strftime('%H:%M:%S', time.localtime())}] has started up")
    ...
    while True:
     try:
        for index, item_list in enumerate(items):
            item_data = await v_one.get_pp(item_list, session, proxy)
            self.search_logs.append(f"V1 {proxy.current} searched {len(item_data)} items")
            self.total_searchers += len(item_list)
            self._total_searchers += len(item_list)
            for item in item_data:
                if not item.get("itemRestrictions"):
        ...
        self.error_logs.append(f"V1 {proxy.current} [{time.strftime('%H:%M:%S', time.localtime())}] {e}")
        open("logs.txt", "a").write(f"\nV1 {proxy.current} [{time.strftime('%H:%M:%S', time.localtime())}] {e}")
        self.total_errors += 1
        self._total_errors += 1
